...
namespace NRKernal
{
    ...
    public class NRHandsManager
    {
        ...
        internal bool StartHandTracking()
        {
            ...
            if (m_HandStatesService != null && m_HandStatesService.StartService())
            {
                NRDebugger.Info("[HandsManager] Hand Tracking Start: Success");
        ...
        private Vector3 GetHandRayDirectionV1(HandState handState)
        {
        ...
        private Vector3 GetHandRayDirectionV2(HandState handState)
        {
            var shoulderPos = GetShoulderPosition(handState.handEnum == HandEnum.RightHand);
            Pose wristPose = handState.GetJointPose(HandJointID.Wrist);
            var forearmpos = wristPose.position + wristPose.rotation * new Vector3(0, -.15f, 0);
            Pose palmPose = handState.GetJointPose(HandJointID.Palm);
            var handpos = palmPose.position;
            var wristOffset = Vector3.Lerp(forearmpos, handpos, 0.25f);
            var rayOrigin = Vector3.Lerp(wristOffset, shoulderPos, 0.56f);
            return (palmPose.position - rayOrigin).normalized;
        }
        ...
        private void CalculatePointerPose(HandState handState)
        {
            if (handState.isTracked)
            {
                ...
                if (handState.pointerPoseValid)
                {
                    ...
                    Vector3 pointerDirection = GetHandRayDirectionV1(handState);
