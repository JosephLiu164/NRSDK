    ...
    public class NRFrame
    {
        ...
        public static EyePoseData EyePoseFromHead
        {
            get
            {
                ...
                if (SessionStatus == SessionState.Running)
                {
                    ...
                    m_EyePoseFromHead.CenterEyePose = NRDevice.Subsystem.GetDevicePoseFromHead(NativeDevice.HEAD_CENTER);
        ...
        internal static void OnPreUpdate(ref LostTrackingReason lostTrackReason)
        {
            ...
            if (NRDebugger.logLevel <= LogLevel.Debug)
                NRDebugger.Info("[NRFrame] OnPreUpdate-XR: LostTrackReason={0}, pose={1}, PresentTime={2}, lastHeadPose={3}, lastPresentTime={4}, frameHandle={5}",
                    lostTrackReason, pose.ToString("F4"), timeStamp, m_HeadPose.ToString("F4"), m_CurrentPoseTimeStamp, NRSessionManager.Instance.NRSwapChainMan.FrameHandle);
        ...
        public static void SetFocusPlane(Vector3 point, Vector3 normal)
        {
...
#endif
            NRSessionManager.Instance.NRSwapChainMan?.SetFocusPlane(point, normal);
        }

        /// <summary> Set a focus point in camera space that acts as the focal point of the Scene for this frame. </summary>
        /// <param name="distance"> The distance of the focal point in the Scene, relative to the Camera.</param>
