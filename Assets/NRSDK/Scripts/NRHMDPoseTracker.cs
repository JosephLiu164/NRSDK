    ...
    public class NRHMDPoseTracker : MonoBehaviour
    {
        ...
        private void OnApplicationPause(bool pause)
        {
            NRDebugger.Info("[NRHMDPoseTracker] OnApplicationPause : pause={0}, headPos={1}, cachedWorldMatrix={2}",
                pause, NRFrame.HeadPose.ToString("F4"), cachedWorldMatrix.ToString());
            if (pause)
            {
        ...
        private IEnumerator Initialize()
        {
            ...
            while (true)
            {
                    ...
                    && NRFrame.GetNodePoseData(nodeStates, XRNode.CenterEye, out centerPose))
                {
                    ...
                    NRDebugger.Info("[NRHMDPoseTracker] XR Init eyePoseFromHead: leftEye={0}, rightEye={1}, centerEye={2}", leftPose.ToString("F4"), rightPose.ToString("F4"), centerPose.ToString("F4"));
                    break;
                }
                else
                {
            ...
            centerAnchor.localPosition = centerCamera.transform.localPosition = eyeposeFromHead.CenterEyePose.position;
            centerAnchor.localRotation = centerCamera.transform.localRotation = eyeposeFromHead.CenterEyePose.rotation;

            NRDebugger.Info("[NRHMDPoseTracker] Init eyePoseFromHead: leftEye={0}, rightEye={1}, centerEye={2}", eyeposeFromHead.LEyePose.ToString("F4"), eyeposeFromHead.REyePose.ToString("F4"), eyeposeFromHead.CenterEyePose.ToString("F4"));
#endif
