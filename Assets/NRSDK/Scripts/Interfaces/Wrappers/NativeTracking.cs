...
namespace NRKernal
{
    ...
    internal partial class NativeTracking
    {
        ...
        public bool Create(UInt64 trackingHandle = 0)
        {
            NRDebugger.Info("[NativeTracking] Create: trackingHandle={0}", trackingHandle);
            if (trackingHandle == 0)
            {
                NativeResult result = NativeApi.NRTrackingCreate(ref trackingHandle);
                NativeErrorListener.Check(result, this, "Create");
            }
            m_TrackingHandle = trackingHandle;
            m_NativeInterface.TrackingHandle = m_TrackingHandle;
            return trackingHandle != 0;
        }
        ...
        public bool Stop()
        {
            ...
            NativeResult result = NativeApi.NRTrackingStop(m_TrackingHandle);
        ...
        private partial struct NativeApi
        {
            ...
            /// <summary> Nr tracking stop. </summary>
            /// <param name="tracking_handle"> Handle of the tracking.</param>
            /// <returns> A NativeResult. </returns>
            [DllImport(NativeConstants.NRNativeLibrary)]
            public static extern NativeResult NRTrackingStop(UInt64 tracking_handle);
