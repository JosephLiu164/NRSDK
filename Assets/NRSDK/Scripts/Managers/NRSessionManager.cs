...
namespace NRKernal
{
    ...
    /// <summary> Values that represent session event types. </summary>
    public enum SessionEventType
    {
        /// <summary> An enum constant representing the session pause. </summary>
        Pause,
        /// <summary> An enum constant representing the session resume. </summary>
        Resume
    }
    ...
    public class NRSessionManager : SingleTon<NRSessionManager>
    {
...
#else
                return NRRenderer.CurrentState == NRRenderer.RendererState.Running;
#endif
            }
        }
        ...
        public void StartSession()
        {
...
#if !UNITY_EDITOR && !USING_XR_SDK
            NRRenderer.Start(NRHMDPoseTracker.leftCamera, NRHMDPoseTracker.rightCamera);
#endif
            ...
            if (config != null)
            {
                ...
                {
                    HandleKernalError(new NRUnSupportDeviceError(string.Format("Unsuppport running on {0}!", deviceType)));
                    return;
                }
            }

            NRHMDPoseTracker.AutoAdaptTrackingType();
            NRDebugger.Info("[NRSessionManager] StartSession: InitTrackingMode={0}", NRHMDPoseTracker.TrackingMode);
